...
export default function ExistingTenantImportList({ imports, loading, total, page, pageSize, processingId, onApprove, onReject, onPage }) {
  ...
  return <section className="space-y-3">
    {imports.map(item => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap gap-2">{item.profile_photo && <a target="_blank" rel="noreferrer" href={item.profile_photo} className="rounded-lg border px-3 py-1.5 text-sm text-indigo-700">View photo</a>}{item.id_proof && <a target="_blank" rel="noreferrer" href={item.id_proof} className="rounded-lg border px-3 py-1.5 text-sm text-indigo-700">View ID proof</a>}</div>
