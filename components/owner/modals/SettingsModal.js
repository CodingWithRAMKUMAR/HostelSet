      ...
      <div className="flex max-h-[calc(100dvh_-_1rem_-_env(safe-area-inset-top)_-_env(safe-area-inset-bottom))] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:max-h-[86dvh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 space-y-2.5 overflow-y-auto px-3 py-3">
          <div>
            ...
            <input type="number" className="h-9 w-full rounded-xl border border-gray-200 px-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100" value={settings.joining_fee ?? ''} onChange={setNumber('joining_fee', 0)} min="0" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-bold text-gray-700">Advance months required</label>
            <input type="number" className="h-9 w-full rounded-xl border border-gray-200 px-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100" value={settings.advance_months ?? ''} onChange={setNumber('advance_months', 0)} min="0" max="12" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="tel" className="h-9 w-full rounded-xl border border-gray-200 px-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100" placeholder="9876543210" value={settings.upi_phone || ''} onChange={(e) => setSettings({ ...settings, upi_phone: e.target.value })} />
        ...
        <div className="flex shrink-0 gap-2 border-t border-slate-200 bg-white p-3 pb-[calc(0.75rem_+_env(safe-area-inset-bottom))]">
          <button onClick={() => onSave(location)} disabled={isSubmitting || !location?.latitude} className="h-9 flex-1 rounded-xl bg-slate-900 text-sm font-bold text-white disabled:opacity-50">
            {isSubmitting ? 'Saving...' : 'Save settings'}
          </button>
          <button onClick={onCancel} className="h-9 flex-1 rounded-xl border border-gray-300 text-sm font-bold text-gray-700">
            Cancel
          </button>
        </div>
      </div>
