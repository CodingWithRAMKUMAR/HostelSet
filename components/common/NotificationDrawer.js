...
export default function NotificationDrawer({ open, onClose }) {
  ...
  const drawer = (
    <div className="fixed inset-0 z-[9999] overflow-hidden" role="dialog" aria-modal="true" aria-label="Notifications">
      ...
      <aside className="fixed right-2 top-2 flex max-h-[calc(100dvh_-_1rem_-_env(safe-area-inset-bottom))] w-[min(380px,calc(100vw_-_1rem))] min-w-0 flex-col overflow-hidden rounded-[1.35rem] border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 sm:right-3 sm:top-3 sm:max-h-[calc(100dvh_-_1.5rem_-_env(safe-area-inset-bottom))] sm:w-[min(420px,calc(100vw_-_1.5rem))]">
        <header className="shrink-0 border-b border-slate-200 p-3 dark:border-slate-700">
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto overflow-x-hidden p-2.5 pb-[calc(1rem_+_env(safe-area-inset-bottom))]">
          ...
          {notifications.map(notification => (
            <button
              ...
              className={`flex w-full min-w-0 gap-2 rounded-xl border p-2.5 text-left shadow-sm transition ${notification.is_read ? 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800' : 'border-orange-200 bg-orange-50 dark:border-orange-500/40 dark:bg-orange-500/10'}`}
            >
