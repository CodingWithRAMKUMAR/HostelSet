...
export default function AdminMobileProperties({ properties = [], loading, avatar = 'A', onBack, onProfile, onView, onArchive, onRestore }) {
  return (
      ...
      {properties.map(property => (
        <article key={property.id} className="min-w-0 rounded-2xl border border-slate-100 bg-white p-2.5 shadow-sm">
          <div className="flex min-w-0 items-start justify-between gap-2">
            ...
            <AdminStatusChip tone={property.archived_at || property.lifecycle_status === 'archived' ? 'amber' : property.is_active ? 'emerald' : 'red'}>{property.archived_at || property.lifecycle_status === 'archived' ? 'Archived' : property.is_active ? 'Active' : 'Inactive'}</AdminStatusChip>
          </div>
          <div className="mt-2 flex min-w-0 items-center justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {property.archived_at || property.lifecycle_status === 'archived' ? (
                <button type="button" onClick={() => onRestore?.(property.id)} className="rounded-lg bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700">Restore</button>
              ) : (
                <button type="button" onClick={() => onArchive?.(property.id)} className="rounded-lg bg-red-50 px-2 py-1 text-xs font-bold text-red-600">Archive</button>
              )}
            </div>
          </div>
        </article>
