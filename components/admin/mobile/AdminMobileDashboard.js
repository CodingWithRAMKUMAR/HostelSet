...
function StatCard({ stat, onNavigate }) {
  ...
  return (
    <Card type={target ? 'button' : undefined} onClick={target ? () => onNavigate(target) : undefined} className="min-w-0 rounded-2xl border border-slate-100 bg-white p-2 text-left shadow-sm">
      <p className="truncate text-[10px] font-semibold uppercase tracking-wide text-slate-400">{stat.label}</p>
      <p className="truncate text-base font-black leading-tight text-slate-900">{stat.value}</p>
    </Card>
  )
}
