...
export function useOwnerPayments(property, tenants, archivedTenants, setStats, loadData, enabled = true) {
  ...
  const loadPayments = async () => {
    ...
    const pendingQuery = supabase.from('payment_history').select('*, tenants(name, phone, room_id, rooms(room_number))').eq('status', 'payment_pending').order('payment_date', { ascending: false });
    ...
    const { data: all, error: allError } = await supabase.from('payment_history').select('*, tenants(name, room_id, rooms(room_number))').in('tenant_id', historyTenantIds).order('payment_date', { ascending: false }).limit(100);
