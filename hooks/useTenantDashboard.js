...
export function useTenantDashboard() {
  ...
  // ATOMIC VACATE CANCELLATION (Fixes the stuck badge)
  const cancelVacateRequest = async () => {
    if (isSubmitting) return; if (!existingVacateRequest) { toast.error('No vacate request to cancel.'); return }
    ...
    if (!confirm('Cancel your vacate request?')) return; setIsSubmitting(true)
    try {
      const { error } = await supabase.rpc('cancel_vacate_request', { p_tenant_id: tenant.id })
    ...
    } catch (error) { console.error('Cancel vacate error:', error); toast.error('Failed to cancel request: ' + error.message); await refreshData(true) }
