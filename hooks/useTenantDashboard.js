...
export function useTenantDashboard() {
  ...
  // 👇 FIX 2: Optimistic Vacate Cancel (Button reverts instantly)
  const cancelVacateRequest = async () => {
    ...
    try {
      // Force UI to revert immediately (optimistic update)
      ...
      if (error) {
        console.warn('RPC error (UI already updated):', error);
      }
  ...
  useEffect(() => {
    ...
    // 👇 FIX 1: Notice Deletion (Removed strict property_id check)
