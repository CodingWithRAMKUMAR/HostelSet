...
export function useTenantDashboard() {
  ...
  const cancelVacateRequest = async () => { if (isSubmitting) return; if (!existingVacateRequest) { toast.error('No vacate request to cancel.'); return } const { data: preBooking } = await supabase.from('pre_bookings').select('id').eq('room_id', tenant.room_id).eq('status', 'approved').maybeSingle(); if (preBooking) { toast.error('Cannot cancel vacate – a pre‑booking has already been approved for this room.'); return } if (!confirm('Cancel your vacate request?')) return; setIsSubmitting(true); try { const { error } = await supabase.rpc('cancel_vacate_request', { p_tenant_id: tenant.id }); if (error) throw error; setExistingVacateRequest(null); setTenant(prev => ({ ...prev, status: 'active', check_out_requested: false, notice_period_start: null, notice_period_end: null })); toast.success('Vacate request cancelled. You remain active.'); await refreshData(true) } catch (error) { console.error('Cancel vacate error:', error); toast.error('Failed to cancel request: ' + error.message); await refreshData(true) } finally { setIsSubmitting(false) } }
