...
export default function AdminDashboard() {
  ...
  const deleteProperty = async (propertyId) => {
    if (!confirm('⚠️ This will permanently delete the property and all related data. This cannot be undone!')) return
  ...
  const postNotice = async () => {
    ...
    if (!propertyId || !title || !content) return toast.error('Missing required fields')
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {(stats.pendingMemberships > 0 || stats.pendingPayments > 0 || stats.pendingApplications > 0 || stats.unresolvedComplaints > 0) && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {stats.pendingMemberships > 0 && <div className="bg-yellow-600/20 border border-yellow-600 text-yellow-300 rounded-xl p-4">⭐ {stats.pendingMemberships} owner(s) without membership</div>}
            {stats.pendingPayments > 0 && <div className="bg-red-600/20 border border-red-600 text-red-300 rounded-xl p-4">💰 {stats.pendingPayments} pending payment confirmations</div>}
            {stats.pendingApplications > 0 && <div className="bg-blue-600/20 border border-blue-600 text-blue-300 rounded-xl p-4">📋 {stats.pendingApplications} new application(s)</div>}
            {stats.unresolvedComplaints > 0 && <div className="bg-orange-600/20 border border-orange-600 text-orange-300 rounded-xl p-4">🔧 {stats.unresolvedComplaints} unresolved complaint(s)</div>}
          </div>
        )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 rounded-2xl p-6 shadow-xl">
              ...
              <div className="space-y-2 text-gray-300">
                ...
                <p>• <strong>Rooms</strong> – edit/delete any room</p>
          ...
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700">
                <tr>
                  <th className="px-4 py-3 text-left"><input type="checkbox" onChange={e => e.target.checked ? setSelectedProperties(filteredProperties) : setSelectedProperties([])} /></th>
                ...
                {paginate(filteredProperties).map(p => (
                  <tr key={p.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-3 flex gap-2">
                      {p.membership_active ? (
                        <button onClick={() => handleMembershipAction(p.owner_id, 'revoke')} className="bg-red-700 hover:bg-red-600 text-white px-3 py-1 rounded-full text-xs">Revoke</button>
                      ) : (
                        <button onClick={() => setGrantModal({ show: true, ownerId: p.owner_id, ownerName: p.users?.full_name || 'Owner' })} className="bg-purple-700 hover:bg-purple-600 text-white px-3 py-1 rounded-full text-xs">Grant</button>
                      )}
                      <button onClick={() => setDeleteConfirmModal({ show: true, type: 'property', id: p.id, name: p.name })} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded-full text-xs">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {activeTab === 'tenants' && (
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700"><tr><th className="px-4 py-3">Name</th><th>Phone</th><th>Room</th><th>Property</th><th>Status</th></tr></thead>
              <tbody>
                {paginate(filteredTenants).map(t => (
                  <tr key={t.id} className="border-b border-gray-800"><td>{t.name}</td><td>{t.phone}</td><td>{t.rooms?.room_number || 'N/A'}</td><td>{t.properties?.name || 'N/A'}</td><td>{t.status}</td></tr>
                ))}
              </tbody>
            </table>
            <div className="flex justify-between p-4">{/* pagination */}</div>
          </div>
        )}
        ...
        {activeTab === 'payments' && (
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700"><tr><th>Date</th><th>Tenant</th><th>Amount</th><th>Method</th><th>Status</th></tr></thead>
              <tbody>
                {paginate(filteredPayments).map(p => (
                  <tr key={p.id} className="border-b border-gray-800"><td>{formatDate(p.payment_date)}</td><td>{p.tenants?.name}</td><td className="text-green-400">{formatCurrency(p.amount)}</td><td>{p.payment_method}</td><td><span className="px-2 py-1 bg-green-800 text-green-200 rounded-full text-xs">{p.status}</span></td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        {activeTab === 'rooms' && (
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700"><tr><th>Room No.</th><th>Property</th><th>Sharing</th><th>Rent (₹)</th><th>Occupancy</th><th>Actions</th></tr></thead>
              <tbody>
                {rooms.map(room => (
                  <tr key={room.id} className="border-b border-gray-800"><td>{room.room_number}</td><td>{room.properties?.name}</td><td>{room.sharing_type}</td><td>{formatCurrency(room.monthly_rent)}</td><td>{room.current_occupants}/{room.capacity}</td><td><button onClick={() => setDeleteConfirmModal({ show: true, type: 'room', id: room.id, name: `Room ${room.room_number}` })} className="text-red-400 text-sm">Delete</button></td></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="space-y-4">
            {preBookings.map(b => (
              <div key={b.id} className="bg-gray-900 rounded-xl border border-gray-800 p-4 flex justify-between items-start">
                ...
                <div className="flex gap-2">
                  <button onClick={() => approvePreBooking(b.id, b.room_id, b.user_id)} className="bg-green-700 hover:bg-green-600 text-white px-3 py-1 rounded text-xs">Approve</button>
                  <button onClick={() => rejectPreBooking(b.id)} className="bg-red-700 hover:bg-red-600 text-white px-3 py-1 rounded text-xs">Reject</button>
                </div>
              </div>
            ))}
        ...
        {activeTab === 'applications' && (
          <div className="space-y-4">
            {applications.map(app => (
              <div key={app.id} className="bg-gray-900 rounded-xl border border-gray-800 p-4">
                ...
                {app.id_proof && <a href={app.id_proof} target="_blank" className="text-purple-400 text-xs">View ID</a>}
              </div>
            ))}
        ...
        {/* ---------- Vacate Tab (with days left) ---------- */}
        {activeTab === 'vacate' && (
          <div className="space-y-4">
            {vacateRequests.map(v => {
              const daysLeft = getDaysUntilVacate(v.expected_check_out)
              return (
                <div key={v.id} className="bg-gray-900 rounded-xl border border-gray-800 p-4 flex justify-between items-center">
                  <div>
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs ${v.status === 'approved' ? 'bg-green-800 text-green-200' : 'bg-yellow-800 text-yellow-200'}`}>{v.status}</span>
                </div>
              )
            })}
        ...
        {activeTab === 'users' && (
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700"><tr><th>Name</th><th>Email</th><th>Phone</th><th>Role</th><th>Actions</th></tr></thead>
              <tbody>
                {users.map(u => (
        ...
        {activeTab === 'owner-settings' && (
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700"><tr><th>Owner</th><th>UPI ID</th><th>UPI Phone</th><th>Joining Fee</th><th>Advance Months</th><th>Actions</th></tr></thead>
              <tbody>
        ...
        {activeTab === 'audit-logs' && (
          <div className="bg-gray-900 rounded-2xl shadow-xl overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700"><tr><th>Time</th><th>Action</th><th>Details</th></tr></thead>
              <tbody>
                {auditLogs.map(log => (
                  <tr key={log.id} className="border-b border-gray-800"><td className="px-4 py-3 text-sm">{formatDate(log.created_at)}</td><td className="font-mono text-sm">{log.action}</td><td className="text-sm">{log.details}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setGrantModal({ show: false, ownerId: null, ownerName: '' })}>
            <div className="bg-gray-900 rounded-2xl max-w-md w-full p-6 border border-gray-800" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3">
                <button onClick={() => handleMembershipAction(grantModal.ownerId, 'grant', grantDuration)} disabled={grantSubmitting} className="flex-1 bg-purple-700 hover:bg-purple-600 text-white py-2 rounded-lg">Grant</button>
                <button onClick={() => setGrantModal({ show: false, ownerId: null, ownerName: '' })} className="flex-1 border border-gray-700 py-2 rounded-lg">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        {editPlanModal.show && editPlanModal.plan && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setEditPlanModal({ show: false, plan: null })}>
            <div className="bg-gray-900 rounded-2xl max-w-md w-full p-6 border border-gray-800" onClick={e => e.stopPropagation()}>
              ...
              <div><label>Price (₹)</label><input type="number" className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 text-white" value={editPlanModal.plan.price} onChange={e => setEditPlanModal({...editPlanModal, plan: {...editPlanModal.plan, price: parseInt(e.target.value)}})} /></div>
              <div><label>Features (comma separated)</label><input type="text" className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 text-white" value={editPlanModal.plan.features?.join(', ')} onChange={e => setEditPlanModal({...editPlanModal, plan: {...editPlanModal.plan, features: e.target.value.split(',').map(f=>f.trim())}})} /></div>
        ...
        {editOwnerSettingsModal.show && editOwnerSettingsModal.settings && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setEditOwnerSettingsModal({ show: false, settings: null })}>
            <div className="bg-gray-900 rounded-2xl max-w-md w-full p-6 border border-gray-800" onClick={e => e.stopPropagation()}>
              ...
              <div><label>UPI ID</label><input type="text" className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 text-white" value={editOwnerSettingsModal.settings.upi_id || ''} onChange={e => setEditOwnerSettingsModal({...editOwnerSettingsModal, settings: {...editOwnerSettingsModal.settings, upi_id: e.target.value}})} /></div>
              <div><label>UPI Phone</label><input type="text" className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 text-white" value={editOwnerSettingsModal.settings.upi_phone || ''} onChange={e => setEditOwnerSettingsModal({...editOwnerSettingsModal, settings: {...editOwnerSettingsModal.settings, upi_phone: e.target.value}})} /></div>
              <div><label>Joining Fee (₹)</label><input type="number" className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 text-white" value={editOwnerSettingsModal.settings.joining_fee || 0} onChange={e => setEditOwnerSettingsModal({...editOwnerSettingsModal, settings: {...editOwnerSettingsModal.settings, joining_fee: parseInt(e.target.value)}})} /></div>
              <div><label>Advance Months</label><input type="number" className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 text-white" value={editOwnerSettingsModal.settings.advance_months || 1} onChange={e => setEditOwnerSettingsModal({...editOwnerSettingsModal, settings: {...editOwnerSettingsModal.settings, advance_months: parseInt(e.target.value)}})} /></div>
