...
export function calculateRentDueStatus(tenant) {
  ...
  if (['inactive', 'archived'].includes(tenant.status)) {
    return { status: 'inactive', message: 'Tenancy inactive', daysUntilDue: null, dueAmount: 0, dueDate: null, urgent: false }
  }
  if (!tenant.move_in_date || !Number(tenant.rent_amount)) {
    return { status: 'unknown', message: 'Due date unavailable', daysUntilDue: null, dueAmount: Number(tenant.pending_amount || 0), dueDate: null, urgent: false }
  }
  ...
  if (daysUntilDue < 0) {
    return { status: 'overdue', message: `Overdue by ${Math.abs(daysUntilDue)} days`, daysUntilDue, dueAmount: pendingAmount, dueDate: expectedDate, urgent: true }
  } else if (daysUntilDue === 0) {
    return { status: 'due_today', message: 'Due today!', daysUntilDue: 0, dueAmount: pendingAmount, dueDate: expectedDate, urgent: true }
  } else if (daysUntilDue <= 5) {
    return { status: 'due_soon', message: `Due in ${daysUntilDue} day${daysUntilDue !== 1 ? 's' : ''}`, daysUntilDue, dueAmount: pendingAmount, dueDate: expectedDate, urgent: true }
  } else {
    return { status: 'pending', message: `Due on ${formatDate(expectedDate)}`, daysUntilDue, dueAmount: pendingAmount, dueDate: expectedDate, urgent: false }
  }
}
