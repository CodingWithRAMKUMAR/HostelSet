...
const SIGNED_DOCUMENT_TTL_MS = 10 * 60 * 1000
const signedDocumentCache = new Map()
const signedDocumentRequests = new Map()
...
const getCachedSignedDocument = (key) => {
  const cached = signedDocumentCache.get(key)
  if (!cached) return null
  if (cached.expiresAt <= Date.now()) {
    signedDocumentCache.delete(key)
    return null
  }
  return cached.url
}

const cacheSignedDocument = (key, url) => {
  if (!url) return
  signedDocumentCache.set(key, { url, expiresAt: Date.now() + SIGNED_DOCUMENT_TTL_MS })
}

const dedupeSignedDocument = async (key, signer) => {
  const cached = getCachedSignedDocument(key)
  if (cached) return cached
  if (signedDocumentRequests.has(key)) return signedDocumentRequests.get(key)
  const request = Promise.resolve()
    .then(signer)
    .then((url) => {
      cacheSignedDocument(key, url)
      return url
    })
    .finally(() => signedDocumentRequests.delete(key))
  signedDocumentRequests.set(key, request)
  return request
}
...
export const clearHostelSetSessionCache = () => {
  ...
  signedDocumentRequests.clear()
}

export const getRestoredSession = async ({ retryDelay = 250 } = {}) => {
  ...
  if (retryDelay > 0 && typeof window !== 'undefined') {
    ...
    if (second?.data?.session || second?.error) return second
  }

  try {
    const refreshed = await supabase.auth.refreshSession()
    if (refreshed?.data?.session || refreshed?.error) return refreshed
  } catch {
    // A missing/expired refresh token is handled by the caller as unauthenticated.
  }
