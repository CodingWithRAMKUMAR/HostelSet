...
export const getPrivateDocumentUrl = async (path, expiresIn = 300) => {
  ...
  const { data, error } = await supabase.storage.from('tenant-documents').createSignedUrl(path, expiresIn)
  if (error) {
...
export const syncServerSession = async (session) => {
  const response = await fetch('/api/auth/session', {
    ...
    headers: session ? { Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json' } : undefined,
