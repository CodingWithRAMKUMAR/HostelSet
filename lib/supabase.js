...
import { normalizePrivateDocumentPath, createExpiringRequestCache } from './privateDocument'
import { logRealtime, syncRealtimeAuth } from './realtime'
...
if (typeof window !== 'undefined' && !window.__hostelsetRealtimeAuthSync) {
  window.__hostelsetRealtimeAuthSync = true
  supabase.auth.getSession()
    .then(({ data: { session } }) => syncRealtimeAuth(supabase, session))
    .catch(() => {})

  supabase.auth.onAuthStateChange((event, session) => {
    if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED' || event === 'USER_UPDATED' || event === 'INITIAL_SESSION') {
      syncRealtimeAuth(supabase, session)
    }
    if (event === 'SIGNED_OUT') syncRealtimeAuth(supabase, null)
  })

  const refreshRealtimeAuth = () => {
    supabase.auth.getSession()
      .then(({ data: { session } }) => syncRealtimeAuth(supabase, session))
      .catch(() => logRealtime('auth-sync-failed', 'reason=session-read'))
  }
  window.addEventListener('online', refreshRealtimeAuth)
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') refreshRealtimeAuth()
  })
}
...
export { normalizePrivateDocumentPath }

export const getPrivateDocumentUrl = async (path, expiresIn = 300) => {
  const objectPath = normalizePrivateDocumentPath(path)
  if (!objectPath) return null

  const { data, error } = await supabase.storage.from('tenant-documents').createSignedUrl(objectPath, expiresIn)
  if (error) {
    if (process.env.NODE_ENV !== 'production') console.warn('[HostelSet] A private document is unavailable.')
    return null
  }
  return data.signedUrl
}

const getExistingImportDocumentUrl = async (importId, field, options = {}) => {
  if (!importId || !['id_proof', 'photo', 'profile_photo'].includes(field)) return null
  const { data: { session } } = await supabase.auth.getSession()
  const userScope = session?.user?.id || 'anonymous'
  return dedupeSignedDocument(`existing-import:${userScope}:${importId}:${field}`, async () => {
    const response = await fetch('/api/owner/import-document-url', {
      ...
      body: JSON.stringify({ importId, documentType: field === 'profile_photo' ? 'photo' : field }),
    })
...
const SIGNED_DOCUMENT_TTL_MS = 4 * 60 * 1000
const MISSING_DOCUMENT_TTL_MS = 2 * 60 * 1000
const signedDocumentCache = createExpiringRequestCache({ successTtlMs: SIGNED_DOCUMENT_TTL_MS, missingTtlMs: MISSING_DOCUMENT_TTL_MS })
const OWNER_PHOTO_TTL_MS = 4 * 60 * 1000
const OWNER_PHOTO_MISSING_TTL_MS = 2 * 60 * 1000
const ownerPhotoCache = new Map()
const ownerPhotoRequests = new Map()
const OWNER_PHOTO_CACHE_LIMIT = 300

function trimOwnerPhotoCache() {
  while (ownerPhotoCache.size > OWNER_PHOTO_CACHE_LIMIT) {
    ownerPhotoCache.delete(ownerPhotoCache.keys().next().value)
  }
}

function ownerTenantPhotoCacheKey(userScope, tenant) {
  return `${userScope}:${tenant.id}:${tenant.profile_photo_path || ''}:${tenant.updated_at || tenant.move_in_date || ''}`
}

export const clearOwnerTenantProfilePhotoCache = () => {
  ownerPhotoCache.clear()
  ownerPhotoRequests.clear()
}

export const getOwnerTenantProfilePhotoUrls = async (tenants = []) => {
  const rows = Array.isArray(tenants) ? tenants.filter(tenant => tenant?.id) : []
  ...
  rows.forEach(tenant => {
    const key = ownerTenantPhotoCacheKey(userScope, tenant)
    const cached = ownerPhotoCache.get(key)
    if (cached && cached.expiresAt > now) {
      if (cached.value) result[tenant.id] = cached.value
      return
    }
    if (cached) ownerPhotoCache.delete(key)
    missing.push(tenant)
    requestKeys.push(key)
  })
  if (!missing.length) return result

  const batchKey = requestKeys.sort().join('|')
  const loader = ownerPhotoRequests.get(batchKey) || (async () => {
    const response = await fetch('/api/owner/tenant-profile-photos', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(session?.access_token ? { Authorization: `Bearer ${session.access_token}` } : {}),
      },
      body: JSON.stringify({ tenantIds: missing.map(tenant => tenant.id) }),
    })
    if (!response.ok) return {}
    const payload = await response.json().catch(() => ({}))
    return payload?.photos || payload?.urls || {}
  })().finally(() => ownerPhotoRequests.delete(batchKey))
  ownerPhotoRequests.set(batchKey, loader)
  const loaded = await loader.catch(() => ({}))
  missing.forEach(tenant => {
    const url = loaded[tenant.id] || null
    ownerPhotoCache.set(ownerTenantPhotoCacheKey(userScope, tenant), {
      value: url,
      expiresAt: Date.now() + (url ? OWNER_PHOTO_TTL_MS : OWNER_PHOTO_MISSING_TTL_MS),
    })
    if (url) result[tenant.id] = url
  })
  trimOwnerPhotoCache()
  return result
}

const getSignedDocumentUserScope = async () => {
  if (typeof window === 'undefined') return 'server'
  const { data: { session } } = await supabase.auth.getSession()
  return session?.user?.id || 'anonymous'
}

const dedupeSignedDocument = async (key, signer, options = {}) => {
  if (options.forceRefresh) signedDocumentCache.delete(key)
  return signedDocumentCache.getOrLoad(key, signer)
}
...
const getApplicationDocumentUrl = async (applicationId, field) => {
  ...
  const { data: { session } } = await supabase.auth.getSession()
  const userScope = session?.user?.id || 'anonymous'
  return dedupeSignedDocument(`application:${userScope}:${applicationId}:${field}`, async () => {
...
export const signPrivateDocumentFields = async (record, fields, options = {}) => {
  if (!record) return record
  const userScope = await getSignedDocumentUserScope()
  const signed = await Promise.all(fields.map(async field => {
    ...
    if (record.source_type === 'existing_tenant_import' && ['id_proof', 'photo', 'profile_photo'].includes(field)) {
      return getExistingImportDocumentUrl(record.id, field, options)
    }
    const path = normalizePrivateDocumentPath(record[field])
    if (!path) return null
    return dedupeSignedDocument(`storage:${userScope}:${field}:${path}`, () => getPrivateDocumentUrl(path), options)
  }))
  return fields.reduce((result, field, index) => ({ ...result, [field]: signed[index] }), { ...record })
}

const isUsableSignedDocumentUrl = value => {
...
export const openSignedPrivateDocument = async (record, field, options = {}) => {
  const previewWindow = typeof window !== 'undefined' ? window.open('', '_blank') : null
  if (previewWindow) {
    try {
      previewWindow.document.title = 'Opening document'
      previewWindow.document.body.innerHTML = '<p style="font-family:system-ui,sans-serif;padding:24px;color:#334155">Opening document...</p>'
    } catch {}
  }
  const signed = await signPrivateDocumentFields(record, [field], { forceRefresh: true, ...options })
  const url = signed?.[field] || null
  if (!isUsableSignedDocumentUrl(url) || typeof window === 'undefined') {
    previewWindow?.close()
    return null
  }
  if (previewWindow) {
...
export const signInWithEmail = async (email, password) => {
  try {
    staleSessionRecoveryComplete = false
...
let syncedSessionKey = null
let sessionRestorePromise = null
let staleSessionRecoveryComplete = false

export const clearHostelSetSessionCache = () => {
  if (typeof window === 'undefined') return
  ;['userId', 'userRole', 'isLoggedIn', 'userName', 'userEmail', 'ownerPropertyId'].forEach(key => {
    window.localStorage.removeItem(key)
  })
  signedDocumentCache.clear()
}

const isStaleSessionError = error => /refresh token|invalid token|session missing|session.*expired/i.test(error?.message || '')

const recoverStaleSessionOnce = async (error) => {
  if (staleSessionRecoveryComplete) return
  staleSessionRecoveryComplete = true
  await supabase.auth.signOut({ scope: 'local' }).catch(() => {})
  clearHostelSetSessionCache()
  await fetchWithTimeout('/api/auth/session', { method:'DELETE' }, 5000).catch(() => {})
  if (process.env.NODE_ENV !== 'production') console.warn('[HostelSet] Stale auth session cleared.', { stage:'restore', status:error?.status || 400 })
}

export const getRestoredSession = ({ retryDelay = 250 } = {}) => {
  if (staleSessionRecoveryComplete) return Promise.resolve({ data:{ session:null }, error:null })
  if (sessionRestorePromise) return sessionRestorePromise
  sessionRestorePromise = (async () => {
    const first = await supabase.auth.getSession()
    if (first?.data?.session) return first
    if (first?.error) {
      if (isStaleSessionError(first.error)) await recoverStaleSessionOnce(first.error)
      return { data:{ session:null }, error:first.error }
    }
    if (retryDelay > 0 && typeof window !== 'undefined') {
      await new Promise(resolve => window.setTimeout(resolve, retryDelay))
      const second = await supabase.auth.getSession()
      if (second?.data?.session) return second
      if (second?.error) {
        if (isStaleSessionError(second.error)) await recoverStaleSessionOnce(second.error)
        return { data:{ session:null }, error:second.error }
      }
    }
    try {
      const refreshed = await supabase.auth.refreshSession()
      if (refreshed?.data?.session) return refreshed
      if (isStaleSessionError(refreshed?.error)) await recoverStaleSessionOnce(refreshed.error)
      return refreshed || first
    } catch (error) {
      if (isStaleSessionError(error)) await recoverStaleSessionOnce(error)
...
export const syncServerSession = async (session) => {
  ...
  const sessionKey = session ? `${accessToken}:${refreshToken}` : 'DELETE'
  if (session && sessionKey === syncedSessionKey) return
  if (sessionSyncPromise?.key === sessionKey) return sessionSyncPromise.promise
  ...
  sessionSyncPromise = { key: sessionKey, promise }
  return promise
}
