...
const MEMBERSHIP_PRODUCT_ID = process.env.DODO_MEMBERSHIP_PRODUCT_ID || 'pdt_0NgsOeKIu8IFQi24BbANl';
const RENT_PRODUCT_ID = process.env.DODO_RENT_PRODUCT_ID;

export async function createDodoOrder({ amount, currency, customerName, customerEmail, purpose, metadata }) {
  try {
    const productId = purpose === 'rent' ? RENT_PRODUCT_ID : MEMBERSHIP_PRODUCT_ID;
    if (!DODO_API_KEY || !productId) throw new Error(`Dodo ${purpose} payments are not configured`);
