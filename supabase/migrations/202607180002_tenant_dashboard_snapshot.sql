-- Reduce the tenant dashboard's initial HTTP fan-out by returning its read-only
-- startup data in one authenticated RPC call. All existing write RPCs, realtime
-- subscriptions, and row-level security policies remain unchanged.
...
as $$
  with current_tenant as (
    ...
    where tenant_row.user_id = auth.uid()
    limit 1
  ),
  ...
  select jsonb_build_object(
    ...
    'payments', (
      ...
      where payment_row.tenant_id = (
        select tenant_row.id from current_tenant as tenant_row limit 1
      )
    ),
    'owner_settings', (
      select jsonb_build_object(
      ...
      where settings_row.property_id = (
        select tenant_row.property_id from current_tenant as tenant_row limit 1
      )
      limit 1
    ),
    'notices', (
...
revoke all on function public.get_my_tenant_dashboard_snapshot() from public;
revoke all on function public.get_my_tenant_dashboard_snapshot() from anon;
grant execute on function public.get_my_tenant_dashboard_snapshot() to authenticated;

comment on function public.get_my_tenant_dashboard_snapshot() is
  'Returns the authenticated tenant dashboard startup snapshot in one RLS-protected request.';
